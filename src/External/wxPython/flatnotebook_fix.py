# copy of wxPython's wx.lib.agw.flatnotebook with fixes that cast floats to ints to avoid reoccuring deprication warnings
...
def PaintStraightGradientBox(dc, rect, startColour, endColour, vertical=True):
    ...
    for i in range(high+1):
        ...
        r = startColour.Red() + int(((i*rd*100)/high)/100)
        g = startColour.Green() + int(((i*gd*100)/high)/100)
        b = startColour.Blue() + int(((i*bd*100)/high)/100)
...
class FNBRendererDefault(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        if hasImage:
            textOffset = int(2*pc._pParent._nPadding + 16 + shapePoints/2)
        else:
            textOffset = int(pc._pParent._nPadding + shapePoints/2)
